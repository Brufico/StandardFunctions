...
cat1 <- function(dataf, nomfact, useNA = "no",
        ...
        num <- sum(tbl$num)
        # compute summaries ordered factor
        # ?? not yet ready
        # bar chart with ggplot2
        pt <- ggplot(if (useNA == "no") {
...
# # ##a
# tp <- cat1(dtf, "nam1", useNA= "no", orderfreq = TRUE, orderdesc = TRUE, cfill = "red")
# tp$plot
# tp$table
# # ##b
# tp <- cat1(dtf, "nam1", useNA= "no", orderfreq = TRUE, orderdesc = TRUE,
...
cat2 <- function(dataf, nomfact1, nomfact2,  useNA = "no",
        ...
        if(orderfreq1 == TRUE & ordervar1 == nomfact2 & !is.na(orderval1)){  # cas de l'ordre des fréquences conditionnelles
                ...
                # gtbl <<- tbl
                # print(tbl) #dbg
                tbl[ , nomfact1] <- orderfact(tbl, nomfact1, orderfreq1, orderdesc1, ordervar = "perc", orderfun = orderfun1) #***************
                # print("succes")
...
#         print(levels(dataf[, nomfact2])) #debug
        ...
        tblcrois <- table(dataf[ , nomfact1], dataf[ , nomfact2], useNA = useNA)
             ...
             #  ?? not yet ready
             #
             #  bar chart with ggplot2 exemple filières + situation-emploi

              # print(dataf) #debug

             pt <- ggplot(if (useNA == "no") {
                     dataf[which(!is.na(dataf[ , nomfact1]) & !is.na(dataf[ , nomfact2]) ), ]} else {dataf},
                    aes_(as.name(nomfact1), fill = as.name(nomfact2)) ) +
